...
e1000_recv(void)
{
  ...
  while (1) {
    ...
    // rx head is initialized as tail + 1,
    // so firstly add 1 to rx tail.
    ...
    if (rx_ring[rx_tail].status & E1000_RXD_STAT_DD) {
      ...
      struct mbuf *m;
      if ((uint64)(m = mbufalloc(0)) == 0) {
        break;
      }

      rx_ring[rx_tail].addr = (uint64)m->head;
      rx_mbufs[rx_tail] = m;
